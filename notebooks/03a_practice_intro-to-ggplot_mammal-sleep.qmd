...
### 1.2 Exploring the Mammals Sleep dataset

We can begin by simply opening the **msleep dataset** by typing `msleep`. However, let's first examine the available metadata for the dataset using the `help()` or `?` functions:
...
We can answer some of these questions using the basic summary functions introduced in notebook 01:
...
Of course, ggplot allows us to create the same thing. The conventional syntax to plot with ggplot is:
...
### 2.3. Bar Plots - Categorical Data
...
## 3. Other Ways to Visualize Distribution
...
::: callout-tip
## 4.1 Exercise - For Continued Practice
