...
```{r} help(msleep) #?msleep    #does the same thing}
```
...
```         
head(dataset_name) class(dataset_name) dim(dataset_name) colnames(dataset_name) str(dataset_name) summary(dataset_name)
```

```{r} head(msleep)   # try "head(msleep, 3)" to see what that does differently}
```
...
```         
ggplot(data=dataset_name, aes(columns assigned to particular visual variables such as x coord, y coord, color fill, color outline, etc.)) +        geom_name(any_additional_arguments)
```
...
```{r} ggplot(data=msleep, aes(x = sleep_total)) +   geom_histogram()   #binwidth=4}
```
...
```{r} ggplot(data=msleep, aes(x = sleep_total)) +   geom_histogram(binwidth=2, color="black", fill="gray")}
```
...
```{r} ggplot(data=msleep, aes(x=vore)) +   geom_histogram(stat="count")  #try without stat = count}
```

We can produce the exact same output with **geom_bar()**.

```{r} ggplot(data=msleep, aes(x=vore)) +   geom_bar(stat="count")}
```
...
```{r} ggplot(msleep, aes(x=fct_infreq(vore), fill=vore)) +   geom_bar(stat="count")}
```
...
```{r} ggplot(msleep, aes(x=order, fill=conservation)) +   geom_bar(stat="count")}
```
